...
if selected==nav_list[2]:
    ...
    link = "./data/env_waspac_linear.csv"
    #link = 'https://raw.githubusercontent.com/FMrtnz/hackathon_code/main/data/env_waspac_linear.csv'
    df = pd.read_csv(link)
...
#Dropdown menus to select the country and the unit

#Defining a dictionnary linking units codes with units names
    units_dict = {'kg per capita':'KG_HAB', 'tons':'T'}
...
#Dropdown menu to select the unit
    ...
    country_choice = st.multiselect('Select countries:',options=  df_chart.loc[units_dict[unit_choice]].reset_index().geo.unique(),default= ['PT', 'FR', 'DE','IT','ES'])


#Generating the bar plot in the chosen unit
    df_multiselect = df_chart.loc[units_dict[unit_choice]].reset_index()
